...
const generatedZodDefinitions = new Set<string>();
...
export function generateTypeScript(
  ...
  const zodSchema = generateZodSchema(doc, name);
  return (shouldIncludeHeader ? header(relativePath) : "") + types + zodSchema;
}
...
export function generateZodSchema(doc: Doc, name: string) {
  ...
  for (const [name, definition] of Object.entries(doc.definitions)) {
    // Skip if we've already generated this Zod definition
    if (generatedZodDefinitions.has(name)) {
      continue;
    }
    generatedZodDefinitions.add(name);
